...
const consumed_products_service_1 = require("./consumed-products.service");
...
describe('ConsumedProductsService', () => {
    ...
    beforeEach(async () => {
        ...
        const module = await testing_1.Test.createTestingModule({
            providers: [
                consumed_products_service_1.ConsumedProductsService,
                {
        ...
        service = module.get(consumed_products_service_1.ConsumedProductsService);
        productsService = module.get(products_service_1.ProductsService);
    });
    // ========================================
    ...
    describe('register', () => {
        ...
        it('deve registrar consumo com sucesso', async () => {
            ...
            productsService.adjustStockWithLocation.mockResolvedValue({ product: mockProduct, movement: {} });
        ...
        it('deve lançar NotFoundException quando produto não existe', async () => {
            ...
            // Act & Assert
            await expect(service.register(registerData)).rejects.toThrow(common_1.NotFoundException);
        ...
        it('deve lançar NotFoundException quando produto pertence a outro salão (multi-tenant)', async () => {
            ...
            productsService.findById.mockResolvedValue(productFromAnotherSalon);
            // Act & Assert
            await expect(service.register(registerData)).rejects.toThrow(common_1.NotFoundException);
            await expect(service.register(registerData)).rejects.toThrow('Produto nao encontrado');
            expect(productsService.adjustStockWithLocation).not.toHaveBeenCalled();
        ...
        it('deve lançar BadRequestException quando produto está inativo', async () => {
            ...
            productsService.findById.mockResolvedValue(inactiveProduct);
            // Act & Assert
            await expect(service.register(registerData)).rejects.toThrow(common_1.BadRequestException);
        ...
        it('deve lançar BadRequestException quando estoque é insuficiente', async () => {
            ...
            productsService.findById.mockResolvedValue(productWithLowStock);
            const dataWithHighQuantity = { ...registerData, quantityUsed: 5 };
            // Act & Assert
            await expect(service.register(dataWithHighQuantity)).rejects.toThrow(common_1.BadRequestException);
    ...
    describe('remove', () => {
        ...
        it('deve converter quantityUsed de string para número ao chamar adjustStockWithLocation', async () => {
            ...
            // Assert
            expect(productsService.adjustStockWithLocation).toHaveBeenCalledWith(expect.objectContaining({
                quantity: 2.5,
            }));
        });
    });
...
//# sourceMappingURL=consumed-products.service.spec.js.map
...
import { AddonsService, QuotaExceededError } from './addons.service';
...
describe('AddonsService', () => {
  ...
  describe('consumeWhatsAppQuota', () => {
    ...
    it('deve consumir 1 quota incluída e decrementar includedRemaining', async () => {
      ...
      // Act
      const result = await service.consumeWhatsAppQuota(
        SALON_ID,
        APPOINTMENT_ID,
    ...
    it('deve retornar sucesso sem consumir quando já foi consumido anteriormente (idempotência)', async () => {
      ...
      // Act - Chamada 1
      const result1 = await service.consumeWhatsAppQuota(
        SALON_ID,
        APPOINTMENT_ID,
        'APPOINTMENT_CONFIRMATION',
      );
    ...
    it('deve lançar erro HTTP 402 quando quota excedida', async () => {
      ...
      // Act & Assert
      try {
        await service.consumeWhatsAppQuota(
          SALON_ID,
          APPOINTMENT_ID,
          'APPOINTMENT_CONFIRMATION',
        );
        ...
        const response = (error as HttpException).getResponse() as QuotaExceededError;

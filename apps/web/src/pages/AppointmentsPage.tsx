      ...
      if (!clientId && (formData.clientName || formData.clientPhone)) {
        try {
          ...
          if (clientError.response?.status === 409 && formData.clientPhone) {
            ...
            // Qualquer erro na criação do cliente (403, 400, etc) - prosseguir sem clientId
            // O appointment aceita clientName/clientPhone diretamente
            console.warn('Erro ao criar cliente, prosseguindo com nome/telefone:', clientError.response?.status);
          }
        }
      }

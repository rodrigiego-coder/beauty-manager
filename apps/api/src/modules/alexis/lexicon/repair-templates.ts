...
import { LexiconEntry, RepairTemplateKey } from './salon-lexicon';
...
const NEXT_QUESTIONS: Record<string, string> = {
  ASK_HAIR_LENGTH_CHEMISTRY:
    'Seu cabelo é curto, médio ou longo? E já fez alguma química recentemente?',
  ASK_BOTOX_INTENT:
    'Quando você fala em botox, é pra alinhar os fios (tipo alisamento leve) ou pra tratamento de reconstrução?',
  ASK_MATIZ_TONE:
    'Você prefere um tom mais pérola (frio) ou bege (quente)?',
  ASK_MECHAS_STYLE:
    'Tem alguma referência de foto? Pode ser babylights (natural) ou balayage (mais marcado).',
  ASK_COLOR_INTENT:
    'Você quer mudar completamente de cor ou retocar o que já tem?',
  ASK_CHEMISTRY_HISTORY:
    'Me conta: seu cabelo tem alguma química (progressiva, coloração, descoloração)?',
  ASK_HAIR_LOSS_DURATION:
    'Faz quanto tempo que notou a queda? E está usando algum produto específico?',
};
 ...
 */
function repairServiceCanonical(ctx: RepairContext): RepairResult {
 ...
 */
function repairConditionToProtocol(ctx: RepairContext): RepairResult {
 ...
 */
function repairAmbiguousAskConfirm(ctx: RepairContext): RepairResult {
  const { entry } = ctx;
...
const TEMPLATE_MAP: Record<RepairTemplateKey, (ctx: RepairContext) => RepairResult> = {
 ...
 */
export function applyRepairTemplate(ctx: RepairContext): RepairResult {
 ...
 */
export function composeRepairResponse(result: RepairResult): string {

...
import { Pool } from 'pg';
import { triageForms, triageQuestions } from '../../database/schema';
...
interface QuestionInput {
  category: 'CHEMICAL_HISTORY' | 'HEALTH_CONDITION' | 'HAIR_INTEGRITY' | 'ALLERGY' | 'CUSTOM';
...
async function seed() {
  ...
  for (const question of DEFAULT_QUESTIONS) {
    await db.insert(triageQuestions).values({
      formId: form.id,
